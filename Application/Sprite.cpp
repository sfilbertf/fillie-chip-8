...
void Application::updateTextureSprite() {
    if(emu.firstNibble != 0xd) return;

    getSpriteData();

    glActiveTexture(textureSprite);
    glBindTexture(GL_TEXTURE_2D, textureSprite);
    uint8_t data[8 * 15 * 3];
...
void Application::sprite() {
    ...
    ImGui::SetNextWindowSize(ImVec2(io.DisplaySize.x * 0.225f, io.DisplaySize.y * 0.3f), ImGuiCond_Always);
